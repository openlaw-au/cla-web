...
var Node = class _Node {
  ...
  @internal
  */
  constructor(type, attrs2, content, marks3 = Mark.none) {
    this.type = type;
    this.attrs = attrs2;
    this.marks = marks3;
  ...
  */
  hasMarkup(type, attrs2, marks3) {
    return this.type == type && compareDeep(this.attrs, attrs2 || type.defaultAttrs || emptyAttrs) && Mark.sameSet(this.marks, marks3 || Mark.none);
  }
  /**
  ...
  */
  static fromJSON(schema4, json) {
    ...
    if (json.marks) {
      ...
      marks3 = json.marks.map(schema4.markFromJSON);
    }
    if (json.type == "text") {
      if (typeof json.text != "string")
        throw new RangeError("Invalid text node in JSON");
      return schema4.text(json.text, marks3);
    }
    let content = Fragment.fromJSON(schema4, json.content);
    return schema4.nodeType(json.type).create(json.attrs, content, marks3);
  }
};
...
var TextNode = class _TextNode extends Node {
  ...
  @internal
  */
  constructor(type, attrs2, content, marks3) {
    super(type, attrs2, null, marks3);
    if (!content)
...
function initAttrs(attrs2) {
  ...
  if (attrs2)
    for (let name in attrs2)
      result[name] = new Attribute(attrs2[name]);
  return result;
}
var NodeType = class _NodeType {
  ...
  */
  ...
  set of marks.
  */
  create(attrs2 = null, content, marks3) {
    if (this.isText)
      throw new Error("NodeType.create can't construct text nodes");
    return new Node(this, this.computeAttrs(attrs2), Fragment.from(content), Mark.setFrom(marks3));
  }
  /**
  ...
  if it doesn't match.
  */
  createChecked(attrs2 = null, content, marks3) {
    content = Fragment.from(content);
    this.checkContent(content);
    return new Node(this, this.computeAttrs(attrs2), content, Mark.setFrom(marks3));
  }
  /**
  ...
  */
  createAndFill(attrs2 = null, content, marks3) {
    ...
    if (!after)
      return null;
    return new Node(this, attrs2, content.append(after), Mark.setFrom(marks3));
  }
  /**
  ...
  */
  ...
  @internal
  */
  static compile(nodes3, schema4) {
    let result = /* @__PURE__ */ Object.create(null);
    nodes3.forEach((name, spec) => result[name] = new _NodeType(name, schema4, spec));
...
var MarkType = class _MarkType {
  ...
  @internal
  */
  constructor(name, rank, schema4, spec) {
    this.name = name;
    this.rank = rank;
    this.schema = schema4;
    this.spec = spec;
  ...
  */
  ...
  @internal
  */
  static compile(marks3, schema4) {
    let result = /* @__PURE__ */ Object.create(null), rank = 0;
    marks3.forEach((name, spec) => result[name] = new _MarkType(name, rank++, schema4, spec));
    return result;
  }
  /**
...
var Schema = class {
  ...
  */
  ...
  nodes.
  */
  node(type, attrs2 = null, content, marks3) {
    ...
    return type.createChecked(attrs2, content, marks3);
  }
  /**
...
function gatherMarks(schema4, marks3) {
  let found2 = [];
  for (let i2 = 0; i2 < marks3.length; i2++) {
    let name = marks3[i2], mark = schema4.marks[name], ok = mark;
    if (mark) {
      found2.push(mark);
    } else {
...
var NodeContext = class {
  constructor(type, attrs2, marks3, pendingMarks, solid, match2, options) {
    this.type = type;
    this.attrs = attrs2;
    this.marks = marks3;
...
var ParseContext = class {
  ...
  withStyleRules(dom, f) {
    ...
    if (!style)
      return f();
    let marks3 = this.readStyles(parseStyles(style));
    if (!marks3)
      return;
    let [addMarks, removeMarks] = marks3, top = this.top;
    ...
    if (this.findPlace(node)) {
      ...
      for (let i2 = 0; i2 < node.marks.length; i2++)
        if (!top.type || top.type.allowsMarkType(node.marks[i2].type))
          marks3 = node.marks[i2].addToSet(marks3);
...
var ReplaceStep = class _ReplaceStep extends Step {
  ...
  */
  static fromJSON(schema4, json) {
    ...
    return new _ReplaceStep(json.from, json.to, Slice.fromJSON(schema4, json.slice), !!json.structure);
  }
};
...
var ReplaceAroundStep = class _ReplaceAroundStep extends Step {
  ...
  */
  static fromJSON(schema4, json) {
    ...
    return new _ReplaceAroundStep(json.from, json.to, json.gapFrom, json.gapTo, Slice.fromJSON(schema4, json.slice), json.insert, !!json.structure);
  }
};
...
function setNodeMarkup(tr, pos, type, attrs2, marks3) {
  let node = tr.doc.nodeAt(pos);
  if (!node)
    throw new RangeError("No node at given position");
  if (!type)
    type = node.type;
  let newNode = type.create(attrs2, null, marks3 || node.marks);
  if (node.isLeaf)
...
var Transform = class {
  ...
  */
  setNodeMarkup(pos, type, attrs2 = null, marks3) {
    setNodeMarkup(this, pos, type, attrs2, marks3);
    return this;
  }
  /**
  ...
  */
  insertText(text2, from2, to) {
    ...
    if (from2 == null) {
      ...
      return this.replaceSelectionWith(schema4.text(text2), true);
    } else {
      if (to == null)
        to = from2;
      to = to == null ? from2 : to;
      ...
      this.replaceRangeWith(from2, to, schema4.text(text2, marks3));
  ...
  updateChildren(view2, pos) {
    ...
    iterDeco(this.node, this.innerDeco, (widget, i2, insideNode) => {
      if (widget.spec.marks)
        ...
        updater.syncToMarks(i2 == this.node.childCount ? Mark.none : this.node.child(i2).marks, inline2, view2);
...
var ViewTreeUpdater = class {
  ...
  syncToMarks(marks3, inline2, view2) {
    ...
    while (depth < marks3.length) {
      ...
      if (found2 > -1) {
        ...
        let markDesc = MarkViewDesc.create(this.top, marks3[depth], inline2, view2);
...
function serializeForClipboard(view2, slice2) {
  ...
  let serializer2 = view2.someProp("clipboardSerializer") || DOMSerializer.fromSchema(view2.state.schema);
...
function parseFromClipboard(view2, text2, html, plainText, $context) {
  ...
  if (asText) {
    ...
    if (parsed) {
      ...
      let { schema: schema4 } = view2.state, serializer2 = DOMSerializer.fromSchema(schema4);
      dom = document.createElement("div");
      text2.split(/(?:\r\n?|\n)+/).forEach((block2) => {
        let p = dom.appendChild(document.createElement("p"));
        if (block2)
          p.appendChild(serializer2.serializeNode(schema4.text(block2, marks3)));
      });
    }
  } else {
  ...
  if (!slice2) {
    let parser2 = view2.someProp("clipboardParser") || view2.someProp("domParser") || DOMParser.fromSchema(view2.state.schema);
      ...
      if (gecko && state.selection.empty && $pos.parentOffset && !$pos.textOffset && $pos.nodeBefore.marks.length) {
        ...
        for (let node = sel.focusNode, offset = sel.focusOffset; node && node.nodeType == 1 && offset != 0; ) {
          ...
          if (before.nodeType == 3) {
            view2.domSelection().collapse(before, before.nodeValue.length);
            break;
          } else {
            node = before;
            offset = -1;
...
handlers.dragstart = (view2, _event) => {
  ...
  let slice2 = (node || view2.state.selection).content(), { dom, text: text2 } = serializeForClipboard(view2, slice2);
  event.dataTransfer.clearData();
  event.dataTransfer.setData(brokenClipboardAPI ? "Text" : "text/html", dom.innerHTML);
  event.dataTransfer.effectAllowed = "copyMove";
  if (!brokenClipboardAPI)
    event.dataTransfer.setData("text/plain", text2);
  view2.dragging = new Dragging(slice2, !event[dragCopyModifier], node);
};
...
function parseBetween(view2, from_, to_) {
  ...
  let parser2 = view2.someProp("domParser") || DOMParser.fromSchema(view2.state.schema);
...
function readDOMChange(view2, from2, to, typeOver, addedNodes) {
  ...
  if (parse.sel) {
    ...
    if (sel2 && !(chrome && android && view2.composing && sel2.empty && (change.start != change.endB || view2.input.lastAndroidDelete < Date.now() - 100) && (sel2.head == chFrom || sel2.head == tr.mapping.map(chTo) - 1) || ie && sel2.empty && sel2.head == chFrom))
...
function encode(string, exclude, keepEscaped) {
  ...
  for (let i2 = 0, l = string.length; i2 < l; i2++) {
    const code3 = string.charCodeAt(i2);
...
Token.prototype.attrIndex = function attrIndex(name) {
  ...
  for (let i2 = 0, len = attrs2.length; i2 < len; i2++) {
    if (attrs2[i2][0] === name) {
      return i2;
    }
  }
  ...
  for (let j = 0, l = blockTokens.length; j < l; j++) {
    ...
    for (let i2 = tokens.length - 1; i2 >= 0; i2--) {
      ...
      if (currentToken.type === "text" && state.md.linkify.test(currentToken.content)) {
        ...
        blockTokens[j].children = tokens = arrayReplaceAt(tokens, i2, nodes3);
      }
    }
  }
}
...
var MarkdownParseState = class {
  constructor(schema4, tokenHandlers2) {
    this.schema = schema4;
    this.tokenHandlers = tokenHandlers2;
    this.stack = [{ type: schema4.topNodeType, attrs: null, content: [], marks: Mark.none }];
  }
  top() {
  ...
  addText(text2) {
    if (!text2)
      return;
    let top = this.top(), nodes3 = top.content, last = nodes3[nodes3.length - 1];
    let node = this.schema.text(text2, top.marks), merged;
    if (last && (merged = maybeMerge(last, node)))
      nodes3[nodes3.length - 1] = merged;
    else
      nodes3.push(node);
  }
...
function tokenHandlers(schema4, tokens) {
  let handlers2 = /* @__PURE__ */ Object.create(null);
  for (let type in tokens) {
    let spec = tokens[type];
    if (spec.block) {
      let nodeType = schema4.nodeType(spec.block);
...
var MarkdownParser = class {
  ...
  */
  constructor(schema4, tokenizer, tokens) {
    this.schema = schema4;
    this.tokenizer = tokenizer;
    this.tokens = tokens;
    this.tokenHandlers = tokenHandlers(schema4, tokens);
  }
  /**
...
var MarkdownSerializer = class {
  ...
  */
  constructor(nodes3, marks3, options = {}) {
    this.nodes = nodes3;
    this.marks = marks3;
    this.options = options;
  }
...
var defaultMarkdownSerializer = new MarkdownSerializer({
  ...
  ordered_list(state, node) {
    ...
    state.renderList(node, space, (i2) => {
      let nStr = String(start + i2);
  ...
  hard_break(state, node, parent, index) {
    for (let i2 = index + 1; i2 < parent.childCount; i2++)
  ...
  */
  text(text2, escape3 = true) {
    ...
    for (let i2 = 0; i2 < lines.length; i2++) {
      this.write();
      if (!escape3 && lines[i2][0] == "[" && /(^|[^\\])\!$/.test(this.out))
        this.out = this.out.slice(0, this.out.length - 1) + "\\!";
      this.out += escape3 ? this.esc(lines[i2], this.atBlockStart) : lines[i2];
  ...
  */
  renderInline(parent, fromBlockStart = true) {
    ...
    let progress = (node, offset, index) => {
      ...
      trailing = "";
      if (node && node.isText && marks3.some((mark) => {
      ...
      let inner = marks3.length ? marks3[marks3.length - 1] : null;
      let noEsc = inner && this.getMark(inner.type.name).escape === false;
      let len = marks3.length - (noEsc ? 1 : 0);
      outer: for (let i2 = 0; i2 < len; i2++) {
        ...
        for (let j = 0; j < active.length; j++) {
          ...
          if (mark.eq(other)) {
            if (i2 > j)
              marks3 = marks3.slice(0, j).concat(mark).concat(marks3.slice(j, i2)).concat(marks3.slice(i2 + 1, len));
            else if (j > i2)
              marks3 = marks3.slice(0, i2).concat(marks3.slice(i2 + 1, j)).concat(mark).concat(marks3.slice(j, len));
            continue outer;
          }
        }
      }
      let keep = 0;
      while (keep < Math.min(active.length, len) && marks3[keep].eq(active[keep]))
        ++keep;
      while (keep < active.length)
...
function keydownHandler(bindings) {
  ...
  return function(view2, event) {
    ...
    if (name.length == 1 && name != " ") {
      ...
      if ((event.shiftKey || event.altKey || event.metaKey || name.charCodeAt(0) > 127) && (baseName = base2[event.keyCode]) && baseName != name) {
...
function chainCommands(...commands) {
  return function(state, dispatch, view2) {
    for (let i2 = 0; i2 < commands.length; i2++)
      if (commands[i2](state, dispatch, view2))
        return true;
    return false;
  };
}
...
var PARA = "\u2029";
var marks2 = schema.spec.marks.addToEnd("smallcaps", {
  parseDOM: [{ tag: "span.clq-sc" }, { style: "font-variant", getAttrs: (v) => /small-caps/.test(v) && null }],
  toDOM() {
    return ["span", { class: "clq-sc" }, 0];
  }
});
var nodes2 = addListNodes(schema.spec.nodes, "paragraph block*", "block").addToEnd("footnote", {
  inline: true,
  group: "inline",
  atom: true,
  selectable: true,
...
var schema3 = new Schema({ nodes: nodes2, marks: marks2 });
var md = lib_default("commonmark", { html: false });
md.inline.ruler.before("emphasis", "clq_footnote", (state, silent) => {
  const src = state.src, start = state.pos;
  if (src.charCodeAt(start) !== 94 || src.charCodeAt(start + 1) !== 91) return false;
...
var parser = new MarkdownParser(schema3, md, {
  blockquote: { block: "blockquote" },
  paragraph: { block: "paragraph" },
  list_item: { block: "list_item" },
...
function serializeDoc(doc3) {
  _fnDefs = [];
  let out = serializer.serialize(doc3);
  if (_fnDefs.length) {
    const defs = _fnDefs.map((t, i2) => {
      const paras = t.split(/\n\s*\n/).map((p) => p.replace(/\s*\n\s*/g, " ").trim());
      return "[^" + (i2 + 1) + "]: " + paras[0] + paras.slice(1).map((p) => "\n\n    " + p).join("");
    });
    out += "\n\n" + defs.join("\n\n");
  }
  return out;
...
function preprocess(text2) {
  ...
  let body = out.join("\n").replace(/\[\^([^\]]+)\]/g, (mm, id) => defs[id.trim()] != null ? "^[" + defs[id.trim()] + "]" : mm);
  return body.replace(/\n{3,}/g, "\n\n").trim();
}
var selectedFnPos = null;
var $fn = document.getElementById("fnText");
var FootnoteView = class {
  constructor(node, view2, getPos) {
    this.node = node;
    this.view = view2;
    this.getPos = getPos;
    ...
    this.dom.addEventListener("mousedown", (e) => {
      e.preventDefault();
      const pos = getPos();
      view2.dispatch(view2.state.tr.setSelection(NodeSelection.create(view2.state.doc, pos)));
      view2.focus();
    });
  }
  update(node) {
  ...
  selectNode() {
    ...
    selectedFnPos = this.getPos();
    $fn.value = this.node.attrs.text;
    $fn.focus();
  }
  deselectNode() {
...
var numberPlugin = new Plugin({
  view() {
    return { update(v) {
      let n = 0;
      v.dom.querySelectorAll("sup.clq-fn").forEach((el) => {
...
function insertFootnote(state, dispatch) {
  const fn = schema3.nodes.footnote.create({ text: "New footnote." });
  if (dispatch) dispatch(state.tr.replaceSelectionWith(fn).scrollIntoView());
  return true;
}
var enter = chainCommands(splitListItem(schema3.nodes.list_item), createParagraphNear, liftEmptyBlock, splitBlock);
var keys2 = {
  "Mod-b": toggleMark(schema3.marks.strong),
  "Mod-i": toggleMark(schema3.marks.em),
  "Shift-Mod-c": toggleMark(schema3.marks.smallcaps),
  "Mod-z": undo,
  "Mod-y": redo,
  "Shift-Mod-z": redo,
  "Enter": enter,
  "Tab": sinkListItem(schema3.nodes.list_item),
  "Shift-Tab": liftListItem(schema3.nodes.list_item)
};
var toolbar = [
  ["Bold", (st) => toggleMark(schema3.marks.strong)(st.state, st.dispatch)],
  ["Italic", (st) => toggleMark(schema3.marks.em)(st.state, st.dispatch)],
  ["SC", (st) => toggleMark(schema3.marks.smallcaps)(st.state, st.dispatch)],
  ["sep"],
  ["H1", (st) => setBlockType2(schema3.nodes.heading, { level: 1 })(st.state, st.dispatch)],
  ["H2", (st) => setBlockType2(schema3.nodes.heading, { level: 2 })(st.state, st.dispatch)],
  ["H3", (st) => setBlockType2(schema3.nodes.heading, { level: 3 })(st.state, st.dispatch)],
  ["\xB6", (st) => setBlockType2(schema3.nodes.paragraph)(st.state, st.dispatch)],
  ["sep"],
  ["Quote", (st) => wrapIn(schema3.nodes.blockquote)(st.state, st.dispatch)],
  ["\u2022 List", (st) => wrapInList(schema3.nodes.bullet_list)(st.state, st.dispatch)],
  ["1. List", (st) => wrapInList(schema3.nodes.ordered_list)(st.state, st.dispatch)],
  ["sep"],
...
for (const item of toolbar) {
  ...
  b.addEventListener("mousedown", (e) => {
    e.preventDefault();
    item[1]({ state: view.state, dispatch: view.dispatch.bind(view) });
    view.focus();
  });
  $tb.appendChild(b);
}
var SAMPLE = `# Recent developments in financial services law
...
function mount(doc3) {
  ...
  const state = EditorState.create({
    doc: doc3,
    plugins: [history(), keymap(keys2), keymap(baseKeymap), numberPlugin]
  });
  view = new EditorView(document.getElementById("editor"), {
    state,
    nodeViews: { footnote: (node, v, getPos) => new FootnoteView(node, v, getPos) }
  });
  window.__clqView = view;
}
function loadMarkdown(text2) {
  mount(parser.parse(preprocess(text2)));
}
try {
  loadMarkdown(SAMPLE);
...
function rememberGh() {
  try {
    localStorage.setItem(LS, JSON.stringify({ repo: $ghRepo.value.trim(), path: $ghPath.value.trim(), branch: $ghBranch.value.trim() }));
  } catch (e) {
  }
}
function ghMsg(text2, cls) {
  $ghMsg.textContent = text2;
...
document.getElementById("ghSave").addEventListener("click", async () => {
  ...
  if (!p.token) {
    ghMsg("A token with repo write scope is required to save.", "err");
    return;
  }
  ghMsg("Saving " + p.path + " \u2026");
  try {
    ...
    const r = await fetch(ghUrl(p.owner, p.name, p.path), {
      method: "PUT",
      headers: { ...ghHeaders(p.token), "Content-Type": "application/json" },
